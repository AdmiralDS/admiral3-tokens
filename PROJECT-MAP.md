...
- экспорт типографики, цветов, радиусов, теней, animation, breakpoints, zIndex и helpers для кастомной primary palette;
...
│   ├── test/                 # Vitest setup
│   └── tokens/               # Pixso export, structured token maps, stories и tests
...
`src/index.ts` является root entrypoint пакета. Он собирает публичные exports из `src/tokens/*`, `src/tokens/themes`, typography API, static token groups и color helpers.
...
Pixso export хранится в `src/tokens/source/local-variables.json`. Он является базовым источником значений для
реализованных token groups, а структурированные TypeScript maps в `src/tokens/<group>` синхронизируются с ним и остаются
источником для публичного API, theme builder и CSS generation.

Синхронизация TS maps выполняется через `npm run sync:tokens`, sync-проверка находится в
`src/tokens/source/localVariables.test.ts`. Новые Pixso-driven группы подключаются в `scripts/sync-tokens.ts` через
отдельный builder и реестр `tokenSyncSteps`; публичные exports, CSS generation и theme builder обновляются отдельно по
контракту конкретной группы.
...
- `CONTRIBUTING.md` - обязательные правила внесения изменений: быстрый старт, workflow обновления токенов из Pixso, commit style, проверки перед PR, release flow, accessibility, порядок добавления новых токенов, Storybook/playground parity.
- `LICENSE` - лицензионный файл пакета.
- `PROJECT-MAP.md` - текущая карта структуры проекта и назначений файлов.
- `README.md` - пользовательская документация пакета: установка, импорты, темы, CSS tokens, framework-agnostic подключение VTB Group UI, customization examples и публичный контракт.
...
- `package.json` - manifest npm-пакета. Описывает имя, версию, `exports`, публикуемые файлы, side effects, scripts, dependencies, peer/dev dependencies, repository metadata и publish config. Основные локальные потоки вынесены в `check:fix`, `check:full`, `storybook`, `playground` и `release`; служебные команды используют префиксы `build:*`, `storybook:*` и `playground:*`. `react` и `styled-components` являются optional peer dependencies интеграционных subpath; основной JS/TS и CSS API остаётся framework-agnostic. Font asset packages лежат в обычных dependencies, чтобы `./fonts` helpers работали без дополнительных установок.
...
- `playground/scenarios/animation.tsx` - сценарий просмотра animation duration/easing tokens.
...
- `scripts/sync-tokens.ts` - генератор TypeScript token maps из `src/tokens/source/local-variables.json` для реализованных Pixso-групп: global colors, theme colors, radius и typography primitives. Новые группы вроде `animation`, `breakpoints` и `zIndex` добавляются отдельными builder-функциями и подключаются в `tokenSyncSteps` после появления стабильной структуры в Pixso export.
...
- Playwright для e2e должен оставаться на версии `1.60.0` или выше из-за известной регрессии установки браузеров в старых версиях.
...
- `sync:tokens` - обновление существующих TypeScript token maps из сохранённого Pixso export.
...
- `src/tokens/source/local-variables.json` - сохранённый Pixso export local variables. Используется как базовый источник значений для реализованных групп токенов.
- `src/tokens/source/localVariables.test.ts` - unit test синхронизации реализованных global colors, theme colors, radius и typography primitives с сохранённым Pixso export.
...
- `src/tokens/breakpoints/index.ts` - public breakpoints barrel. Экспортирует breakpoint token map и типы.
- `src/tokens/breakpoints/breakpoints.ts` - source of truth для breakpoint tokens.

## Tokens: zIndex

- `src/tokens/zIndex/index.ts` - public zIndex barrel. Экспортирует zIndex token map и типы.
- `src/tokens/zIndex/zIndex.ts` - source of truth для semantic z-index layer tokens.
